...
@section('css')
    <!-- Si usás AdminLTE (Bootstrap 4) no es necesario volver a cargar Bootstrap 5.
         Si decidís mantener los enlaces que tenías, asegurate que no haya conflicto de versiones. -->
                        ...
                        <div class="col-md-4 mb-4">
                            <div class="card card-tomo">
                                ...
                                <div class="card-footer d-flex flex-column gap-2">
                                    {{-- Opción 3: mostrar siempre botón Reactivar si el tomo está inactivo --}}
                                    @if(!$tomo->activo)
                                        <button
                                          type="button"
                                          class="btn btn-sm btn-success"
                                          data-toggle="modal"
                                          data-target="#modalReactivate-{{ $tomo->id }}"
                                    ...
                                    @else
                                        <!-- Botón Información -->
                                        <button type="button" class="btn btn-sm btn-info" data-toggle="modal" data-target="#modalInfo-{{ $tomo->id }}" data-bs-toggle="modal" data-bs-target="#modalInfo-{{ $tomo->id }}">
                                            <i class="fas fa-info-circle"></i> Información
                                        </button>
                                        <!-- Botón Editar -->
                                        <button type="button" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#modalEdit-{{ $tomo->id }}" data-bs-toggle="modal" data-bs-target="#modalEdit-{{ $tomo->id }}">
                                            <i class="fas fa-edit"></i> Editar
                                        </button>
                                        <!-- Botón Eliminar -->
                                        <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#modalDelete-{{ $tomo->id }}" data-bs-toggle="modal" data-bs-target="#modalDelete-{{ $tomo->id }}">
...
@section('js')
    <!-- Scripts: no recargar otra versión de Bootstrap si usás AdminLTE (Bootstrap 4) -->
    ...
    <script>
      $(document).ready(function(){
        // Filtros dinámicos (idénticos a los tuyos)
